    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if mode == "list":
                while len(contacts) < num_contacts:
                    ...
                    for card in results:
                        ...
                        try:
                            card.click()
                            page.wait_for_selector("h1.DUwDvf", timeout=10000)  # ✅ business name
                        except Exception:
                            continue
                        ...
                        # Go back to results
                        try:
                            page.locator('button[aria-label="Back"]').click(timeout=3000)
                        except Exception:
                            try:
                                page.go_back()
            ...
            if mode == "profile" or not contacts:
                try:
                    card_name = page.locator("h1.DUwDvf").first.text_content()
                    details = extract_details(page, card_name, search_query)
                    contacts.append(details)
                except Exception:
...
def extract_details(page, card_name, source_query):
    ...
    try:
        details["company_name"] = page.locator("h1.DUwDvf").first.text_content()
    except:
        pass
    try:
